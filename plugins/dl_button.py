...
async def ddl_call_back(bot, update):
    ...
    tmp_directory_for_each_user = (
        f"{DOWNLOAD_LOCATION}/{str(update.from_user.id)}"
    )
    ...
    if os.path.exists(download_directory):
        save_ytdl_json_path = (
            f"{DOWNLOAD_LOCATION}/{str(update.message.chat.id)}.json"
        )
        download_location = f"{DOWNLOAD_LOCATION}/{update.from_user.id}.jpg"
...
async def download_coroutine(bot: Union[Client, None], session: ClientSession, url: str, file_name: str, chat_id: Union[str, int, None], message_id: Union[int, None], start: float, headers: dict):
    downloaded = 0
    display_message = ""
    async with session.get(url, timeout=PROCESS_MAX_TIMEOUT, headers=headers) as response:
